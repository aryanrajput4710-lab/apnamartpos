const fs = require('fs');
const filepath = 'client/src/pages/Orders.jsx';
...
;
content = content.replace("const [pagination, setPagination] = useState({});", "const [pagination, setPagination] = useState({});" + stateVars);
...
;
content = content.replace("const fetchOrders =", logicFuncs + "\n  const fetchOrders =");
...
;
content = content.replace(/<td style=\{\{ padding: '0\.75rem' \}\}>\{o\.status\}<\/td>\s*<td style=\{\{ padding: '0\.75rem' \}\}>\{o\.user\.name\}<\/td>\s*<td style=\{\{ padding: '0\.75rem' \}\}>\s*<Link to=\{\/orders\/\$\{o\.id\}\\}>View Details<\/Link>\s*<\/td>/g, tableRowReplacement);
